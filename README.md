...
运行中的 CCI 可在操作列表中选择“保存为镜像”：选择容器、CCR 命名空间和镜像名称，确认后提交。平台会暂停容器数分钟，完成后自动恢复；镜像容量须在 100 GB 内。通过“镜像快照”刷新状态，成功后可复制完整镜像地址，用于新建 CCI / ACP。版本标签由平台生成。此功能保存容器文件系统，与“复制”创建模板不同；不保存运行中的进程状态。保存前请清理容器中的密钥等敏感文件，挂载的 AFS 数据请另行备份。使用工具的默认 SSH 启动脚本创建新实例时，会重新生成该实例的 SSH 主机密钥并写入所选公钥；未重建的容器再次执行启动脚本时复用主机密钥；容器重建会生成新密钥。接口与验证范围见 [镜像快照说明](docs/CCI-SNAPSHOTS.md)。

...
上传镜像需要正在运行的 Docker。先选择可访问且 ACTIVE 的命名空间，再从本地镜像列表选择源镜像（可搜索名称或标签，也可手动填写名称 / ID），目标名称和标签默认从所选镜像提取，仅替换 Registry 和命名空间，回车即可保留，也可修改。通过镜像 ID 上传时需填写目标名称。已有可用 Docker 凭据时不重复询问密码；推送权限由 Registry 校验。

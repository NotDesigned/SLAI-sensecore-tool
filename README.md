...
各级编号菜单统一使用 `0` 返回，主菜单的 `0` 为退出。确认页使用 `0. 取消`，默认仍为取消；旧 `q` 输入兼容保留，但不再显示为菜单选项。文字输入中的 `0` 保留为普通输入，可用 `q` 取消。
...
已完成官方 CLI/API 调研和当前账号的任务列表、详情只读验证，详见 [ACP 调研与接入方案](docs/ACP-INTEGRATION.md)。ACP 菜单尚未接入；建议作为独立训练任务服务，复用工作空间、资源池、规格、镜像及 AFS 选择，保留 Job/Worker 与 CCI App/Service 的差异。

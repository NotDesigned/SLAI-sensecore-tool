...
服务列表加载期间仍可创建或上传，也可返回。查看详情或连接后保留列表，提交修改后才自动刷新；r 可手动读取最新状态。“按照上次配置”入口始终可见，尚无记录时会提示先正常创建。

资源操作菜单返回、关闭详情或 SSH 信息后会直接回到列表，不再经过“资源操作已结束”页。取消未提交操作、账户或工作空间配置成功后也直接返回；错误、写入结果待确认，以及保存配置后的文件路径仍会显示。
...
列表每页 20 条，←→ 翻页，`/` 搜索，Enter 查询，`r` 刷新。Textual 中 CCI/ACP 列表缓存 15 秒、DNAT 缓存 30 秒，重新进入先显示会话缓存并在后台更新；缓存按账户、工作空间和筛选范围隔离。写入后缓存失效，操作前仍实时复查目标。选中资源后进入操作。查询在后台进行，等待时可以返回。详情的“复制全部”写入系统剪贴板；无法使用剪贴板时可“保存文本”后打开复制。

不使用全屏界面时运行 `uv run main.py --text`；列表加 `--plain` 可直接打印结果。

完整使用示例：[CCI 开发、保存镜像、ACP 跑 MNIST、再用 CCI 查看共享日志](examples/mnist/README.md)。
...
运行中的 CCI 可在操作列表中选择“保存为镜像”：选择容器、CCR 命名空间和镜像名称，确认后提交。平台会暂停容器数分钟，完成后自动恢复；镜像容量须在 100 GB 内。通过“已保存的镜像”刷新状态，成功后可复制完整镜像地址，用于新建 CCI / ACP。版本标签由平台生成。此功能保存容器文件系统，与“复制”创建模板不同；不保存运行中的进程状态。保存前请清理容器中的密钥等敏感文件，挂载的 AFS 数据请另行备份。使用工具的默认 SSH 启动脚本创建新实例时，会重新生成该实例的 SSH 主机密钥并写入所选公钥；未重建的容器再次执行启动脚本时复用主机密钥；容器重建会生成新密钥。接口与验证范围见 [保存镜像说明](docs/CCI-SNAPSHOTS.md)。
...
默认镜像为预装 sshd 的 `ccr-zhicheng-02/slai-cci-pytorch-ssh:25.06-20260911`，已验证 CCI 登录；需要该命名空间的拉取权限。默认 SSH 配置包含服务就绪检查。自定义镜像若缺少 sshd，启动脚本仍会尝试通过 apt 安装；预装镜像的构建说明见 [CCI 镜像](images/cci/README.md)。已停止的 CCI 可在列表选择“启动”，继续使用原来的 CCI 名称和配置。实测停止再启动后容器临时文件不保留，需先保存为镜像或写入 AFS；复制用于另建一台 CCI，保留原模板和存储，不自动迁移 DNAT。

CCI 列表中选择运行中的实例会直接显示操作菜单；点击“连接”后才查找同可用区、同 VPC 的 TCP 单端口 DNAT 入口。当前列表会话内入口查询结果缓存 30 秒；每次连接仍实时核对 CCI 和选中规则的目标、端口，检测 SSH 响应后显示命令。多个入口可按 IP:端口选择，默认优先容器端口 22。没有入口时会提示先在 DNAT 服务绑定规则。自定义端口需要容器实际提供 SSH 服务。
...
镜像列表表示“当前账号可访问的命名空间”，不等于本人创建。CCR 首次查询可能返回整个仓库目录。列表与 CCI/ACP 镜像选择共享磁盘缓存：5 分钟内直接复用；Textual 界面可先展示 24 小时以内的过期缓存，并在后台更新，标明缓存时间和状态。启动界面及进入创建配置表时只预取默认/当前镜像命名空间，不扫描全部仓库。搜索、翻页使用已有快照；点击“更新云端”（或 r）在后台更新，仍可操作旧列表。完成后自动更新显示并保留焦点、选中行；失败保留旧结果，自动重试退避 30 秒，手动更新可立即重试。上传成功后自动清除对应命名空间的缓存，取消上传则复用缓存。缓存按账户和完整命名空间范围隔离，保存在忽略提交的 `.cache/ccr/`，不包含账户密钥。

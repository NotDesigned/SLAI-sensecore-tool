...
资源操作包装页在正常导航结束、已查看详情、写入成功后自动关闭；成功写入以通知反馈并刷新列表。未提交的取消独立于错误处理，直接回到上一层。仅保存配置等尚未展示的文本结果保留包装页；写入后中断、接口错误、账户已保存但工作空间未完成等情况保留结果页面，避免隐藏未确认状态。
...
列表最多渲染当前 20 条。ACP 按服务端分页和筛选读取；CCI/DNAT 使用完整快照并在本地筛选。Textual 的三类列表共享最多 64 项会话缓存，CCI/ACP 有效期 15 秒、DNAT 30 秒，旧数据最多保留 5 分钟；过期后台刷新，不阻塞显示。资源列表最多并发 2 个后台查询，失败后退避 30 秒。CCR 使用命名空间完整快照，因为当前服务端忽略分页。CCR 快照持久化至 `.cache/ccr/`，账户凭据仅参与单向摘要分区，不写入缓存正文；不同账户、区域和命名空间身份隔离。
...
CCR 磁盘快照支持先显示过期缓存再后台更新（最多保留 24 小时）；前台 peek 不等待网络锁，并复用最多 16 份按文件状态校验的已解析快照。同一命名空间合并后台请求，最多并发 2 路；预取仅针对默认或当前镜像命名空间。失效操作递增版本并删除缓存文件，晚到查询不能把旧结果写回。浏览器按版本变化更新当前页，不抢占输入焦点、不提交尚未按 Enter 的搜索条件。

这些后台策略用于 Textual；纯文本和 --plain 命令保持同步读取和明确返回，镜像可复用仍在有效期内的磁盘缓存。

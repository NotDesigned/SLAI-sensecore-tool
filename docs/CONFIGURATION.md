...
ACP 默认闲时资源，CCI 默认预留资源。所选资源池还需允许对应配额；例如调试池可能只允许预留资源，服务端拒绝时界面会提示切换资源池或配额。资源池与规格下拉按表格对齐显示，表头在上、每行一个候选项。资源池列出可用区、`reserved_number`（接口称“剩余卡数”）和 `spot_status` 里的闲时额度；这两个数是接口分开报的两种量，**不是把剩余卡数按预留/闲时拆开**，也不应相加。接口未返回、值不可解析、或多条闲时额度里有一条读不出来时，该列显示 `-`，不记为 0 也不少算。`reserved_number` 在接口文档里标为 beta，且随时间漂移；它只用于挑池参考，不参与任何校验，配额不足仍以服务端拒绝为准。`[acp.last]` 保存镜像、启动方式、命令、资源和挂载，复用时重新校验。

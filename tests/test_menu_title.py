...
class MenuTitleTests(unittest.TestCase):
    def test_account_cached_but_workspace_reloaded(self):
        config={'sco':{'access_key_id':'ak','access_key_secret':'sk'},'workspace':{'name':'first'}}
        cache={}
        with patch.object(cli,'load_config',return_value=config), patch.object(rest,'get_json',return_value={'username':'L202500224'}) as get:
            ...
            config['sco']['access_key_id']='other'
            cli.menu_title(cache)
            self.assertEqual(get.call_count,2)

    def test_unconfigured_and_unavailable(self):
        with patch.object(cli,'load_config',return_value={'sco':{}}), patch.object(rest,'get_json') as get:
            self.assertIn('用户：未配置 · 工作空间：未选择',cli.menu_title({}))
            get.assert_not_called()
        with patch.object(cli,'load_config',return_value={'sco':{'access_key_id':'ak','access_key_secret':'sk'}}), patch.object(rest,'get_json',side_effect=cli.ConfigError('secret')):
